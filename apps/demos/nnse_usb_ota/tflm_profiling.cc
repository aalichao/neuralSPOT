...
TfLiteStatus tflm_profiling_run(ns_model_state_t *model_state) {
    if (model_state == NULL) {
        ns_lp_printf("Model state is NULL.\n");
        return kTfLiteError;
    }
    ...
    model_state->interpreter->Invoke(); // Characterization via TFLM microProfiler
    
    #ifndef AM_PART_APOLLO5B
        ns_stop_perf_profiler(); // Stop the profiler
    #endif // AM_PART_APOLLO5B

    model_state->profiler->LogCsv(); // prints and also captures events in a buffer
    ...
    ns_set_power_monitor_state(2);
        ...
        ns_lp_printf("\nPMU profiling .\n");
        ns_parse_pmu_stats(num_layers, model_state->rv_count); // Parse the PMU stats and print them out in CSV format
    #endif // AM_PART_APOLLO5B
